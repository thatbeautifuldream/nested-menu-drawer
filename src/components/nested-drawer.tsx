...
const EASING = [0.32, 0.72, 0, 1] as const; // Smooth easing for all animations
...
export function NestedDrawer({ children, initialMenu }: TNestedDrawerProps) {
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Escape to close or go back (only when open)
...
function Trigger({ children, className, ...props }: TTriggerProps) {
  ...
  return (
    <button
      className={cn(
        "relative flex h-10 flex-shrink-0 items-center justify-center gap-2 overflow-hidden rounded-full bg-card px-4 text-sm font-medium shadow-sm transition-all hover:bg-accent border",
        className
      )}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed -inset-8 bg-black/40 z-50 backdrop-blur-sm"
